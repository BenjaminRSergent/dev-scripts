...
def record_procs(file, filter_set):
    total_mhz_usage = 0.0
    total_mhz = 0.0
    num_samples = 0
    with open(file, 'w+') as proc_file:
        ...
        while not os.path.exists(STOP_RECORDING_FILE):
            filtered_procs = filter_procs(get_normalized_stats(), filter_set)
            total_mhz += get_ave_cpu_freq()
            total_mhz_usage += sum(proc.get_stat(process_info.StatType.CPU) for proc in filtered_procs)
            num_samples = num_samples + 1
            time_to_use = str(time.time())
            proc_file.write(time_to_use + "\n")
            record_proc_to_file(proc_file, filtered_procs)
            time.sleep(0.1)
